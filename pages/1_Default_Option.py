...
st.set_page_config(page_title = "Math Major - Default Option", layout = "wide")
st.title("Default Option")
...
# Comm section
def comm_section():
    global com_tc, com_credits, com111z, spe321, wri121z, com_choice1, com_choice2, ud_credits 
    #ud_credits = 0
...
# Physics option
def phy_section():
    global phy_tc, phy_credits, phy221, phy222, phy223, phy1_prefix, phy1_cn, phy2_prefix, phy2_cn, ud_credits
    ...
    #st.write(hum1_cn)
    if phy1_cn != None:
        if int(''.join(filter(str.isdigit, phy1_cn))) >= 300:
            ud_credits += int(phy1_credit)
    if phy2_cn != None:
        if int(''.join(filter(str.isdigit, phy2_cn))) >= 300:
            ud_credits += int(phy2_credit)


# Core math section
def math_section():
    global math_tc, math_credits, stat201, math251, math252, math253, math254, math310, math321, math322, math341, math346, math354, math421, math451, math465, math_choice1, math_choice2, ud_credits
    ...
    with ud_math_col:
        st.write("#### Upper Division Math (44 credits)")
        ...
        if uploaded_df["math"][15] == "":
            ...
            math_choice1_index = ["Choose No Class", "MATH 422 - Applied Partial Differential Equations II", "MATH 423 - Applied Partial Differential Equations III", "MATH 452 - Numerical Methods II", "MATH 453 - Numerical Methods III"].index(uploaded_df["math"][15])
    
        if uploaded_df["math"][16] == "":
            math_choice2_index = None
        else:
            math_choice2_index = ["Choose No Class", "MATH 422 - Applied Partial Differential Equations II", "MATH 423 - Applied Partial Differential Equations III", "MATH 452 - Numerical Methods II", "MATH 453 - Numerical Methods III"].index(uploaded_df["math"][16])
        
        math_choice1 = st.selectbox("Choose one class", ["Choose No Class", "MATH 422 - Applied Partial Differential Equations II", "MATH 423 - Applied Partial Differential Equations III", "MATH 452 - Numerical Methods II", "MATH 453 - Numerical Methods III"], index = math_choice1_index, placeholder = "Pick One", key = "math_choice1")
        math_choice2 = st.selectbox("Choose one class", ["Choose No Class", "MATH 422 - Applied Partial Differential Equations II", "MATH 423 - Applied Partial Differential Equations III", "MATH 452 - Numerical Methods II", "MATH 453 - Numerical Methods III"], index = math_choice2_index, placeholder = "Pick One", key = "math_choice2")
...
def upper_div_section():
    # Upper div electives
    global ue_tc, ue_credits, ue1_prefix, ue1_cn, ue2_prefix, ue2_cn, ud_credits
    st.write("### Upper Division (300+) Electives (7 Credits)")
    #st.write(uploaded_df)
    if uploaded_file == None:
        ue_tc = st.selectbox("Upper Division Electives Transfer Credits (300+)", list(range(0, 76)))
    else:
        ue_tc = st.selectbox("Upper Division Electives Transfer Credits (300+)", list(range(0, 76)), index=int(uploaded_df["ue"][0]))
    st.write("Specific Elective Classes (300+)")
    ...
    # Upper div credits
    ud_credits += int(ue1_credit) + int(ue2_credit)
    


def focused_elec_seq_section():
    global fes_tc, fes_credits, fes1_prefix, fes1_cn, fes2_prefix, fes2_cn, fes3_prefix, fes3_cn, ud_credits
    ...
    #st.write(hum1_cn)
    if fes1_cn != None:
        if int(''.join(filter(str.isdigit, fes1_cn))) >= 300:
            ud_credits += int(fes1_credit)
    if fes2_cn != None:
        if int(''.join(filter(str.isdigit, fes2_cn))) >= 300:
            ud_credits += int(fes2_credit)
    if fes3_cn != None:
        if int(''.join(filter(str.isdigit, fes3_cn))) >= 300:
            ud_credits += int(fes3_credit)

# Focused elective seq
def focused_ele_section():
    global fe_tc, fe_credits, fe1_prefix, fe1_cn, fe2_prefix, fe2_cn, ud_credits
    ...
    #st.write(hum1_cn)
    if fe1_cn != None:
        if int(''.join(filter(str.isdigit, fe1_cn))) >= 300:
            ud_credits += int(fe1_credit)
    if fe2_cn != None:
        if int(''.join(filter(str.isdigit, fe2_cn))) >= 300:
            ud_credits += int(fe2_credit)
...
with st.sidebar.expander("Total General Education Credits: " + str(com_credits + hum_credits + ss_credits + phy_credits) + " (" + str(58 - (com_credits + hum_credits + ss_credits + phy_credits)) + " needed)"):
...
with st.expander("Analysis and Download"):
    ...
    credits_data = pd.DataFrame(
                    {
                        "Sections" : ["Comm", "Hum", "Soc Sci", "Phy", "Math", "Low Ele", "Upper Elec.", "Foc Elec. Seq.", "Foc Elec."],
