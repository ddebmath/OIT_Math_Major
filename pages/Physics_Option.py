...
st.set_page_config(page_title = "Math Major - Physics Option", layout = "wide")
st.title("Physics Option")
...
phy223_value = True if uploaded_df["or"][1] == "True" else False
phy311_value = True if uploaded_df["or"][2] == "True" else False
engr211_value = True if uploaded_df["or"][3] == "True" else False
ee221_value = True if uploaded_df["or"][4] == "True" else False

math342_value = True if uploaded_df["oe"][3] == "True" else False
engr212_value = True if uploaded_df["oe"][4] == "True" else False
engr318_value = True if uploaded_df["oe"][5] == "True" else False
engr355_value = True if uploaded_df["oe"][6] == "True" else False
ee223_value = True if uploaded_df["oe"][7] == "True" else False
ee225_value = True if uploaded_df["oe"][8] == "True" else False
ee341_value = True if uploaded_df["oe"][9] == "True" else False
...
def option_req_section():
    global or_tc, or_credits, phy223, phy311, engr211, ee221, ud_credits
    
    st.write("### Option Specific Classes (Required) (15 Credits)")
    if uploaded_file == None:
    ...
    phy311 = st.checkbox("PHY 311 - Introduction to Modern Physics", value = phy311_value)
    engr211 = st.checkbox("ENGR 211 - Engineering Mechanics: Statics", value = engr211_value) 
    ee221 = st.checkbox("EE 221 - Circuits I", value = ee221_value)

        
    or_credits = or_tc + 4*phy223 + 3*phy311 + 4*engr211 + 4*ee221
...
def option_elec_section():
    global oe_tc, oe_credits, oe1_prefix, oe1_cn, math342, engr212, engr318, engr355, ee223, ee225, ee341, ud_credits
    ...
    # Class 1
    st.write("Specific Elective Classes (At least one PHY 300+ but not PHY 311)")
    oe_col1, oe_col2 = st.columns(2)
    with oe_col1:
        if uploaded_file == None or (uploaded_file != None and uploaded_df["oe"][1] == ""):
            oe1_prefix = st.selectbox("Prefix", list(cp), index = None, placeholder = "Pick One", key = "oe1_prefix")
        else:
            oe1_prefix = st.selectbox("Prefix", list(cp), index = int(list(cp).index(str(uploaded_df["oe"][1]))), placeholder = "Pick One", key = "oe1_prefix1")
        df_oe1 = df[df['Prefix'] == oe1_prefix]
        #st.write(df_ue2)
    with oe_col2: 
        if uploaded_file != None and uploaded_df["oe"][2] != "":
            oe1_cn = st.selectbox("Number", list(df_oe1["Code"]), key = "oe1_cn1", index = int(list(df_oe1["Code"]).index(uploaded_df["oe"][2]))) 
        else:
            oe1_cn = st.selectbox("Number", list(df_oe1["Code"]), key = "oe1_cn") 

    # Elective classes
    math342 = st.checkbox("MATH 342 - Linear Algebra II", value = math342_value)
    engr212 = st.checkbox("ENGR 212 - Engineering Mechanics: Dynamics", value = engr212_value)
    engr318 = st.checkbox("ENGR 318 - Engineering Mechanics: Fluids", value = engr318_value) 
    engr355 = st.checkbox("ENGR 355 - Thermodynamics", value = engr355_value)
    ee223 = st.checkbox("EE 223 - Circuits II", value = ee223_value)
    ee225 = st.checkbox("EE 225 - Circuits III", value = ee225_value)
    ee341 = st.checkbox("EE 341 - Electricty & Magentism with Transmission", value = ee341_value)

    # Find the credit hours for each elective 
    oe1_credit = df_oe1[df_oe1["Code"] == oe1_cn]["Credit Hours:"]


    # This is if nothing is chosen then the credits defaults to 0
    if oe1_prefix == None or oe1_cn == None:
        oe1_credit = 0


    oe_credits = oe_tc + int(oe1_credit) + 4*math342 + 3*engr212 + 4*engr318 + 3*engr355 + 4*ee223 + 4*ee225 + 4*ee341
    st.write("###### :green[Total Focused Elective Sequence Credits:] " + str(f":green[{oe_credits}]"))

    # Upper div credits
    ud_credits += 4*math342 + 4*engr318 + 3*engr355 + 4*ee341
    if oe1_cn != None:
        if int(''.join(filter(str.isdigit, oe1_cn))) >= 300:
            ud_credits += int(oe1_credit)
...
def free_ele_section():
    global le_tc, le_credits, le1_prefix, le1_cn, le2_prefix, le2_cn, le3_prefix, le3_cn, le4_prefix, le4_cn, le5_prefix, le5_cn, le6_prefix, le6_cn, le7_prefix, le7_cn, le8_prefix, le8_cn, le9_prefix, le9_cn, le10_prefix, le10_cn, \
     ...
     # Lower div elective
    st.write("### Free Electives (42 Credits)")
    if uploaded_file == None:
        le_tc = st.selectbox("Free Electives Transfer Credits", list(range(0, 76)))
    else:
        le_tc = st.selectbox("Free Electives Transfer Credits", list(range(0, 76)), index=int(uploaded_df["le"][0]))
    ...
    le_credits = le_tc + int(le1_credit) + int(le2_credit) + int(le3_credit) + int(le4_credit) + int(le5_credit) + int(le6_credit) + int(le7_credit) + int(le8_credit) + int(le9_credit) + int(le10_credit) + int(le11_credit)
...
with st.sidebar.expander("#### Total Option Specific Credits: " + str(or_credits + senseq_credits + oe_credits + le_credits) + " (" + str(35-(or_credits + senseq_credits + oe_credits + le_credits)) + " needed)"):
    st.write("#### Total Option Specific Classes (Required) Credits: " + str(or_credits) + " (" + str(15-or_credits) + " needed)")
...
st.sidebar.write("#### Total Free Electives Credits: " + str(le_credits) + " (" + str(42-le_credits) + " needed)")
...
with st.expander("Analysis and Download"):
    ...
    created_data = {"comm" : [com_tc, com111z, spe321, wri121z, com1_prefix, com1_cn, com_choice1, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None],
                    ...
                    "or" : [or_tc, phy223, phy311, engr211, ee221, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None],
                    ...
                    "oe" : [oe_tc, oe1_prefix, oe1_cn, math342, engr212, engr318, engr355, ee223, ee225, ee341, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None],
